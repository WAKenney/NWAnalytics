...
def aggFilter(agData):
    ...
    gb.configure_column(field='description', header_name='Tree Description',
        editable=False, filter = True, wrapText=True, autoHeight = True)
    ...
    gb.configure_column(field='diversity_level', hide = False)
...
def pivTable(ptab):
    """Setup a pivot table for more detailed data exploration"""
...
def diversity(data):
    ...
    divPivotButton = st.button(f"Click here to see a table of ALL entries at the {divLevel} level and their frequency.")
        
    if divPivotButton:

        diversity_pivot = pd.pivot_table(data, index = divLevel, values = 'tree_name', aggfunc='count')
        diversity_pivot.rename(columns = {'tree_name': 'Frequency'},inplace = True)
        st.dataframe(diversity_pivot, width = 400)

        #this provides the optiont o save the species list as a CSV file for further use
        @st.cache_data()
        def convert_df(df):
            return df.to_csv().encode('utf-8')

        csv = convert_df(diversity_pivot)

        st.download_button(
            label="Download this list as a CSV file that can be read by most spreadsheet software.",
            data=csv,
            file_name= divLevel + 'List.csv',
            mime='text/csv',
                            )
    ...
    with st.expander("Click to view tabular data for the pie chart.", expanded=False):
...
def speciesOrigin(data):
    ...
    # with st.expander("Click here to see a map of the Ecodistrict.", expanded=False):
